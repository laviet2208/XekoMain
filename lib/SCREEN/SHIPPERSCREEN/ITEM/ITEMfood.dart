...
class ITEMfood extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Future<void> changeShipper(accountNormal ship) async {
      try {
        DatabaseReference databaseRef = FirebaseDatabase.instance.reference();
        await databaseRef.child('Order/foodOrder/' + order.id + '/shipper').set(ship.toJson());
      } catch (error) {
        print('Đã xảy ra lỗi khi đẩy catchOrder: $error');
        throw error;
      }
    ...
    String destination = '';
    destination = order.locationGet.firstText + ' , ' + order.locationGet.secondaryText.toString();
          ...
          Padding(
            ...
            child: Container(
              ...
              child: Container(
                ...
                child: ButtonType1(Height: (order.status == 'B' || order.status == 'C') ? 35 : 0, Width: width-40, color: Color.fromARGB(255, 0, 177, 79), radiusBorder: 5, title: acceptText, fontText: 'arial', colorText: Colors.white,
                    onTap: () async {
                      if (((order.cost * (order.costFee.discount/100)) + (order.shipcost * (order.costBiker.discount/100))) <= currentAccount.totalMoney) {
                        if (order.status == 'B') {
                          toastMessage('đang nhận đơn');
                          await changeShipper(currentAccount);
                          await changeMoney(currentAccount.totalMoney - (order.cost * (order.costFee.discount/100)) - (order.shipcost * (order.costBiker.discount/100)));
